...
2. 分模型总 Token 必须保留独立可见列，优先取服务商总数；缺失时按有效输入与输出 Token 求和，不重复计入缓存 Token。
3. 可用缓存读取占比保持不变；原本为 N/A/未知时，自动使用该模型命中缓存读取 Token 总数 ÷ 总输入 Token 数。缺少分量、分母为零、负数或缓存读取大于输入时保持未知，不捏造 0%。
